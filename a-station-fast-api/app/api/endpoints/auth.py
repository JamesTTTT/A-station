...
from app.crud.user_crud import create_user, get_user, get_user_by_email, user_exists
...
async def login_user(
    ...
    refresh_token_data = create_refresh_token(
        user_id=user.id, device_info=device_info, ip_address=ip_address, db=db
    )
...
async def logout_all_devices(
    ...
    return {"message": f"Logged out from all devices", "sessions_revoked": count}
...
async def refresh_token(
    ...
    if not db_token:
        ...
        if (
            potential_token
            and potential_token.revoked
            and potential_token.replaced_by_token_id
        ):
            grace_period = timedelta(seconds=30)
            if (
                potential_token.updated_at
                and (datetime.now(timezone.utc) - potential_token.updated_at)
                < grace_period
            ):
                new_token = (
    ...
    revoke_refresh_token(db, token_id=db_token.id, replaced_by_id=new_db_token.id)
